        ...
        <div className="grid gap-8 md:grid-cols-2">
          {SITE_DATA.teaching.map((course, idx) => (
            <div key={idx} className="card rounded-xl border overflow-hidden hover:shadow-lg transition-all duration-300 group">
              <div className="h-2 bg-[var(--steel)] w-full  transition-colors"></div>
              <div className="p-8">
                <div className="flex justify-between items-start mb-4">
                  <span className="font-mono text-sm font-bold bg-[var(--panel)] text-cyan-100 px-3 py-1.5 rounded-lg border border-[var(--border)]">
                    {course.code}
                  </span>
                ...
                <h3 className="font-bold text-slate-50 text-2xl mb-2 group-hover:text-[var(--ink)] transition-colors">
                  {t(course.name)}
                </h3>
